...
Item{
    ...
    function post()
    {
        if(contentField.text!=""){
            ...
            var data="newsid="+mysid+"&commentNick="+settings.getValue("name","undefined")+"&commentContent="+contentField.text+settings.getValue("signature","----我的小尾巴")
            var other="&parentCommentID="+parentCommentID+"&txtCode=undefined&type=comment"//&client="+settings.getValue("client","1")+"&device="+settings.getValue("device","RM-821")
    ...
    Keys.onPressed: {
        ...
        if(event.key===16842752)
        {
            if(contentField.activeFocus){
                post()
            }else contentField.forceActiveFocus()
        }
